...
if __name__ == '__main__':
    ...
    net = tfext.stlnet.Stlnet(**params)

    with net.graph.as_default():
        cross_entropy = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=net.fc_cliques, labels=net.y_gt))
        update_ops = net.graph.get_collection(tf.GraphKeys.UPDATE_OPS)
        assert len(update_ops) > 0
        with tf.control_dependencies(update_ops):
            train_step = tf.train.AdagradOptimizer(learning_rate=1e-3,
