...
class Alexnet(object):
    ...
    def __init__(self, init_model=None, num_classes=1000,
        ...
        self.y_gt = tf.placeholder(tf.int32, shape=(None,))
        with tf.device(self.device_id):
            ...
            fc6_dropout = tf.nn.dropout(self.fc6_relu, self.fc6_keep_prob)
            ...
            tr_vars['fc7w'], tr_vars['fc7b'] = \
                self.get_fc_weights(layer_index, net_data, num_inputs, num_outputs)
            layer_index += 1
            self.fc7 = tf.matmul(fc6_dropout, tr_vars['fc7w']) + tr_vars['fc7b']
            self.fc7_relu = tf.nn.relu(self.fc7, name='fc7_relu')

            self.fc7_keep_prob = tf.placeholder_with_default(1.0, tuple())
            fc7_dropout = tf.nn.dropout(self.fc7_relu, self.fc7_keep_prob)
            ...
            tr_vars['fc8w'], tr_vars['fc8b'] = \
                self.get_fc_weights(layer_index, net_data, num_inputs, num_outputs)
            layer_index += 1
            self.fc8 = tf.matmul(fc7_dropout, tr_vars['fc8w']) + tr_vars['fc8b']
    ...
    def get_conv_weights(self, layer_index, net_data, kernel_height, kernel_width,
        ...
        if layer_index >= self.num_layers_to_init or net_data is None:
            ...
            w = tf.Variable(net_data[l_name][0])
            b = tf.Variable(net_data[l_name][1])
        return w, b
    ...
    def conv(input, kernel, biases, kernel_height, kernel_width,
        ...
        def convolve(inp, w):
            return tf.nn.conv2d(inp, w, [1, s_h, s_w, 1], padding=padding)
